...
public class SourceCardViewModel : ObservableObject
{
    ...
    /// <summary>Bubbled from a chip's right-click "Change timestamp pattern..." - the owner
    /// (MainWindow) opens the LogType editor jumped straight to the graphical timestamp picker,
    /// pre-seeded with this binding's real files.</summary>
    public event Action<SourceCardViewModel, SourceLogType, LogType>? ChangeTimestampRequested;

    /// <summary>Bubbled from a chip's right-click "Delete Log Type entirely..." - the owner
    /// (MainViewModel/MainWindow) runs the same cross-source cascade delete used by the LogTypes
    /// window, since deleting a LogType affects every source it's bound to, not just this card.</summary>
    public event Action<SourceCardViewModel, LogType>? DeleteLogTypeRequested;
    ...
    private void RebuildChips()
    {
        ...
        foreach (var binding in Source.LogTypes)
        {
            ...
            var chip = new LogTypeChipViewModel(binding, logType, dropCommand, cancelCommand);
            chip.ChangeTimestampRequested += c => ChangeTimestampRequested?.Invoke(this, c.Binding, c.LogType);
            chip.RemoveFromSourceRequested += c => RemoveBindingFromSource(c.Binding, c.LogType);
            chip.DeleteLogTypeRequested += c => DeleteLogTypeRequested?.Invoke(this, c.LogType);
    ...
    /// <summary>"Remove from this source" - unbinds one LogType from just this Source (its
    /// binding on any other Source, and the LogType definition itself, are untouched). Deletes
    /// this binding's already-loaded rows for this source only, mirroring what removing a whole
    /// Source already does at the Source level.</summary>
    private void RemoveBindingFromSource(SourceLogType binding, LogType logType)
    {
        var confirm = MessageBox.Show(
            $"Remove \"{logType.Name}\" from \"{Source.Name}\"? Its {binding.ParsedBlockCount:N0} already-loaded row(s) for this source will be deleted. " +
            "The Log Type itself (and its files on any other source) won't be affected. This cannot be undone.",
    ...
    public async Task SyncBindingAsync(SourceLogType binding, LogType logType)
    {
        ...
        try
        {
            ...
            var actionable = result.Warnings.FirstOrDefault(w => w.RequiresUserAction);
            binding.HasError = actionable is not null;
            binding.ErrorMessage = actionable?.Reason ?? string.Empty;
