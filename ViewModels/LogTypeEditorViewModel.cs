...
public class LogTypeEditorViewModel : ObservableObject
{
    ...
    public IReadOnlyList<(string Key, string Label)> IconChoices => LogType.IconChoices;
    ...
    /// <summary>True once this editor was opened via a LogType chip's right-click "Change
    /// timestamp pattern..." - the whole point of that entry point is to go straight to the
    /// graphical picker, so it bypasses the normal Failed/Ambiguous gate below.</summary>
    private readonly bool _startInManualOverrideMode;

    public bool ShowTimestampPicker => !IsDelimitedType && (_startInManualOverrideMode || IsFailed || (IsAmbiguous && ShowManualOverride));
    ...
    /// <param name="startInManualOverrideMode">True for the "Change timestamp pattern..." entry
    /// point reached by right-clicking a LogType chip - forces the graphical timestamp picker
    /// (or, for a delimited type, the manual column-override panel) to be shown immediately once
    /// detection runs, regardless of whether detection came back Confident/Ambiguous/Failed, so
    /// the user always lands straight on the "wizard" they asked to use rather than needing
    /// detection to fail first.</param>
    public LogTypeEditorViewModel(LogType? existing, IEnumerable<string>? initialSampleFiles = null, bool startInManualOverrideMode = false)
    {
        ...
        _startInManualOverrideMode = startInManualOverrideMode;
