...
public class MainViewModel : ObservableObject
{
    ...
    /// <summary>Bubbled up from a chip's right-click "Change timestamp pattern..." - the owner
    /// (MainWindow) opens the LogType editor jumped straight to the graphical timestamp picker.</summary>
    public event Action<SourceCardViewModel, SourceLogType, LogType>? ChangeTimestampRequestedForBinding;

    /// <summary>Bubbled up from a chip's right-click "Delete Log Type entirely..." - the owner
    /// (MainWindow) runs the same cross-source cascade delete used by the LogTypes window.</summary>
    ...
    private void AddSourceCardInternal(LogSource source)
    {
        ...
        card.ChangeTimestampRequested += (c, binding, logType) => ChangeTimestampRequestedForBinding?.Invoke(c, binding, logType);
        card.DeleteLogTypeRequested += (c, logType) => DeleteLogTypeRequested?.Invoke(logType);
